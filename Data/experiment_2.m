function[soc_cover, katz_cover, deg_cover, random_cover, ...
    top_kaz, top_soc_k, top_deg, install, test_nodes] = experiment_2(G,A)
...
parfor i=1:ntrials
    ...
    if no_start(node) == 0
        [i,i]
        test_nodes(i) = node; 
        [soc_cover_val, ~] = delta_soc_2(G, [node], 1,  steps, niter2, install);
        [katz_cover_val, ~] = delta_soc_2(G, [node], 1, steps, niter2, install);
        [deg_cover_val, ~] = delta_soc_2(G, [node], 1, steps, niter2, install);
        [rand_cover_val,~] = delta_soc_2(G, [node], 1, steps, niter2, install);
