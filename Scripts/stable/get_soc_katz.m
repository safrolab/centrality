function [soc_katz] = get_soc_katz( A, steps, ...
                                        labelled_nodes, p )
%GET_SOC_KATZ Return SOC-Katz centrality 
%   stable_version
%   Input:
%   A - adj matrix
%   labelled_nodes - installation nodes
%   steps - full soc
%   p - spread probability/damping factor
       
A(A > 0) = 1;
[x,y] = size(labelled_nodes);
num_labelled_nodes = max(x,y);
[~, n] = size(A);
I = speye(n);
J = sparse(n,n);
D = A*ones(n,1);
...
a2 = max(a1,a2);
if p > a2
    fprintf('damping factor is too large, using %s', a2);
end
a2 = min(p, a2);
%alpha = a2;
%a2 = 0.8;
