...
def soc_betweenness_centrality(G, soc_nodes, steps, install_units, k=None, 
    ...
    for node_s in nodes:
        ...
        S, P, sigma, D = _single_source_shortest_path_basic(G, soc_nodes, s,
                                                            steps,
                                                            install_units)
        # accumulation
        #if endpoints:
...
def fixed_source_soc_betweenness_centrality(G, soc_nodes, steps, install_units,
                                            source_nodes, normalized=True, 
                                            endpoints=False, seed=None):
    betweenness = dict.fromkeys(soc_nodes, 0.0)
    for node_s in source_nodes:
        s = (node_s, steps-1)  # start at full soc
        S, P, sigma, D = _single_source_shortest_path_basic(G, soc_nodes,
                                                            s, steps,
                                                            install_units)
        betweenness = _accumulate_basic(betweenness, S, P, sigma, s, D)
    return betweenness
...
def betweenness_centrality(G, steps, install_nodes):
    soc_nodes = []
    install_units = {}
    for node in G.nodes():
        install_units[node] = False
        soc_nodes.append((node, 'terminal'))
        for soc_level in range(steps):
            soc_nodes.append((node, soc_level))
...
def fixed_source_betweenness_centrality(G, steps, install_nodes, source_nodes):
    soc_nodes = []
    install_units = {}
    for node in G.nodes():
        install_units[node] = False
    ...
    for node in install_nodes:
        install_units[node] = True
    soc_betweenness = fixed_source_soc_betweenness_centrality(
        G, soc_nodes, steps, install_units, source_nodes, normalized=True,
        endpoints=False, seed=None
        )
    betweenness = {}
    for node, soc_level in soc_betweenness:
...
def draw_graph(graph, install_nodes, mybetweenness,
               standard_betweenness, cordfile):
    ...
    if cordfile:
        ...
        nodes = nx.draw_networkx_nodes(G, pos, node_color=m_betweenness,
                                       node_size =node_sizes,
                                       cmap=plt.cm.Blues,
                                       linewidths=0, with_labels=False)
        pos2 = {}
        for node in install_nodes:
            pos2[node] = pos[node]
        btn2 = [mybetweenness[node] for node in install_graph.nodes()]
        size2 = [node_sizes[node-1] for node in install_graph.nodes()]
        nodes2 = nx.draw_networkx_nodes(install_graph, pos2,node_color=btn2,
                                        node_size =size2, cmap=plt.cm.Blues,
                                        vmin=min(m_betweenness),
                                        vmax=max(m_betweenness),
                                        linewidths=0.8)
...
def debug():
    ...
    betweenness = betweenness_centrality(G, steps, install_units)
...
def expected_btn_centrality(graph, G_n_soc_nodes, steps, k):
    ...
    for _ in range(1000):
      ...
      mybetweenness = betweenness_centrality(G, steps, install_nodes)
